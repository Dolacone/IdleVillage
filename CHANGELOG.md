...
- 試煉目標改為每 25000 點一個可選級距。
- 每種扣款資源保留 10000。
- 試煉目標選項超過 25 項時使用分頁。
- 試煉開啟流程使用原子交易避免並發重複扣款。

## 2026-08-09

- 村莊試煉達成公告每行欄位順序調整，由「玩家名稱：貢獻 X，獲得 Y 個」改成「貢獻 X (Y 素材)：玩家名稱」，跨行仍依貢獻降冪排序。

## 2026-08-08

- 村莊試煉達成公告不再用 `<@user_id>` mention 標示參與者，改成顯示玩家名稱（先查 Discord member 快取，查無再即時解析，皆失敗才退回顯示 user_id）。

## 2026-07-20

- 自動工具改為「隨用隨扣」素材：啟動時只扣 1 個該工具素材（涵蓋第一個小時），之後每小時開始再扣 1 個；不再一次預付全部。
- 某個小時要扣素材時手上沒有該素材，自動工具立即中斷並釋放工具。
- 自動工具剩餘運行時間改為玩家自訂、與素材脫鉤：運行中可隨時加時間（不超過上限）或減時間，減到底即停止。
- 自動工具最長運行時間上限由 6 小時提高到 24 小時。
- 自動工具子介面改版：啟動選初始運行時數，運行中提供「加時間 / 減時間」下拉，並顯示手上素材可再撐幾小時。
...
- Added a village trial system: a "🏆 開啟試煉" button on `/idlevillage` (disabled unless a trial can currently be started, including "no resource type can afford it") instantly opens a global village trial with a fixed target of `TRIAL_TARGET_AMOUNT` (default 50000) — no player input at all. The village resource type spent is chosen automatically (uniformly at random among food/wood/knowledge, whichever can currently afford the amount); the opener's identity is not recorded or announced. While active, every player's action output (any type, full/partial cycles, and burst) counts toward the same shared progress counter, running in parallel with and independent of the stage system. Reaching the target within `TRIAL_DURATION_SECONDS` (default 12h) splits the target/100 universal material among participants by contribution ratio, each share individually rounded up; missing the deadline forfeits the spent resource (no refund) and blocks a new trial for `TRIAL_COOLDOWN_SECONDS` (default 12h). Dashboard and `/idlevillage` show trial progress and personal contribution, without naming the resource type (to avoid implying the goal is a single-resource collection target); public notifications fire on start, success, and failure.
- `.env` is now optional: any key left blank or missing falls back to its default value in `.env.example` (previously the app required every key to be explicitly set).
