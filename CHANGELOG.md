...
- Added a village trial system: any player can spend X of a chosen village resource (food/wood/knowledge, X a multiple of 1000), via a "🏆 開啟試煉" button on `/idlevillage` (disabled unless a trial can currently be started) that opens a Modal for the resource type and target, to open a global village trial with target X. While active, every player's action output (any type, full/partial cycles, and burst) counts toward the same shared progress counter, running in parallel with and independent of the stage system. Reaching the target within 24h splits `X/100` universal material among participants by contribution ratio, each share individually rounded up; missing the deadline forfeits the spent resource (no refund) and blocks a new trial for 12h. Dashboard and `/idlevillage` show trial progress and personal contribution, without naming the resource type (to avoid implying the goal is a single-resource collection target); public notifications fire on start, success, and failure.
