...
管理全服單一的「村莊試煉」：玩家花費村莊資源開啟一個限時目標，所有玩家的行動產出（不分類型）計入同一進度，達標依貢獻度分配萬能素材。與 `stage-manager` 的五關循環系統完全獨立、並行運作，兩者互不影響。
...
| `resource_type` | text / null | null | 開啟試煉時花費的資源類型（`food`/`wood`/`knowledge`） |
| `target` | int | 0 | 試煉目標值（= 開啟時花費的資源數量 X） |
| `progress` | int | 0 | 目前累積進度 |
| `started_at` | timestamp / null | null | 試煉開始時間 |
| `ended_at` | timestamp / null | null | 上一次試煉結束時間（成功或失敗皆更新），用於 12 小時冷卻判定 |

同時維護一張 `trial_contributions` 表，記錄「當前試煉」每位玩家的累積貢獻（`user_id` 唯一鍵）。開啟新試煉時清空此表；試煉結束（成功或失敗）後也清空。

## 開啟試煉

`start_trial(db, resource_type, target, user_id, now)`：

前置條件（不滿足則 raise ValueError，不扣除任何資源）：
- `resource_type` 必須是 `food`/`wood`/`knowledge` 之一
- `target >= TRIAL_TARGET_STEP` 且 `target % TRIAL_TARGET_STEP == 0`
- 目前沒有進行中的試煉（`is_active == 0`）
- 冷卻已過：`ended_at is null` 或 `now - ended_at >= TRIAL_COOLDOWN_SECONDS`
- 村莊 `resource_type` 資源池 `>= target`

成立後：
1. 從村莊資源池扣除 `target` 個 `resource_type`（呼叫 `resource-manager.withdraw`，不退還機制）。
2. 清空 `trial_contributions`。
3. 寫入 `trial_state`：`is_active=1, resource_type, target, progress=0, started_at=now`（`ended_at` 不變）。
...
- `get_trial_info(db)` — 回傳 `trial_state` 完整 dict。供沒有既有 fetch-helper 模式的呼叫端使用（例如 `actions.py` 的 `open_trial_start`/`modal_start_trial` 前置條件驗證、`trial_manager` 內部）。Dashboard／`/idlevillage` 主介面的資料擷取層（`notification.py`／`actions.py`／`general.py`）比照其查詢 `stage_state`／`buildings`／`village_resources` 的既有慣例，直接以 `SELECT * FROM trial_state WHERE id=1` 查詢，不透過此函式。
...
| 變數 | 預設 | 說明 |
| :--- | :--- | :--- |
| `TRIAL_DURATION_SECONDS` | 86400（24 小時） | 試煉開始後的達標期限 |
| `TRIAL_COOLDOWN_SECONDS` | 43200（12 小時） | 試煉結束（成功或失敗）後，多久才能開啟新試煉 |
| `TRIAL_TARGET_STEP` | 1000 | 試煉目標值必須為此值的整數倍 |
