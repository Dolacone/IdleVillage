...
- 功能範圍:
  ...
  - 使用 Token 啟動村莊保護。
  - 設定村莊命令。
- 批量觸發規則 (v2026.04.16.00):
  - 個人加成與村莊保護支援單次選取 1, 2, 3, 5, 10, 25, 50 個代幣進行批量觸發。
  - 介面必須提供數量選取下拉選單 (Quantity Dropdown)。
  - 若所選數量大於玩家持有量, 送出必須失敗, 且不可扣除任何代幣或改動既有效期。
- 個人加成說明:
  - 必須清楚說明對應類別行動在公式計算時獲得 `+100` 總素質。
  - 必須標註 1 個代幣提供 3 週期加成。
  - 若目前已有同類加成, 需依所選數量延長既有效期。
  - 若目前已有異類加成, 需明確以新類型覆蓋, 並依所選數量從使用當下重設效期。
- 村莊保護說明:
